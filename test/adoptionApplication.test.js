...
describe('Adoption Application API', () => {
    ...
    test('POST /api/adoption-applications should create a new adoption application',
        async () => {
            ...
            expect(res.body.applicantdate).toBe(newApplication.applicantdate);
            expect(res.body.statusAdoption).toBe(newApplication.statusAdoption);
            expect(res.body.comments).toBe(newApplication.comments);
        });
    ...
    //test to get adoption application by id
    test('GET /api/adoption-applications/:id should return adoption application by ID',
        async () => {
            const newApplication = { applicantdate: '2023-10-01', statusAdoption: 'Pending', comments: 'Looking forward to adopting!' };
            ...
            const getRes = await request(app).get(`/api/adoption-applications/${appId}`);

            expect(getRes.statusCode).toBe(200);
            expect(getRes.body).toHaveProperty('id', appId);
            expect(getRes.body.applicantdate).toBe(newApplication.applicantdate);
            expect(getRes.body.statusAdoption).toBe(newApplication.statusAdoption);
            expect(getRes.body.comments).toBe(newApplication.comments);
        });

    //test to get adoption application by invalid id
    test('GET /api/adoption-applications/:id should return 400 for invalid ID',
        async () => {
            const getRes = await request(app).get('/api/adoption-applications/invalid-id');

            expect(getRes.statusCode).toBe(400);
            expect(getRes.body).toHaveProperty('message', 'Invalid ID');
        });
    
    //test to get adoption application when not found
    test('GET /api/adoption-applications/:id should return 404 if application not found',
        async () => {
            const getRes = await request(app).get('/api/adoption-applications/9999');

            expect(getRes.statusCode).toBe(404);
            expect(getRes.body).toHaveProperty('message', 'Adoption application not found');
        });
});
