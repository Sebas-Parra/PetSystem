...
describe('Pets CRUD - separated tests', () => {
  ...
  test('POST /api/pets creates a pet', async () => {
    const newPet = { nombre: 'Fido', edad: 3, especie: 'Perro' };
  ...
  test('GET /api/pets returns a list', async () => {
    await request(app).post('/api/pets').send({ nombre: 'A',  especie: 'Gato' }).expect(201);
    await request(app).post('/api/pets').send({ nombre: 'B', especie: 'Perro' }).expect(201);
  ...
  test('GET /api/pets/:id returns the pet', async () => {
    const created = await request(app).post('/api/pets').send({ nombre: 'Solo',  especie: 'Perro' }).expect(201);
  ...
  test('PUT /api/pets/:id updates the pet', async () => {
    const created = await request(app).post('/api/pets').send({ nombre: 'Up', edad: 1,  especie: 'Perro' }).expect(201);
    const id = created.body.id;
  ...
  test('DELETE /api/pets/:id deletes the pet', async () => {
    const created = await request(app).post('/api/pets').send({ nombre: 'Del',  especie: 'Perro' }).expect(201);
    const id = created.body.id;
